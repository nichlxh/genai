...
- Cloud Hosted: Deployed web link temporarily removed.
- Docker:
  ...
  4) Enter the below command to run the image:
     ...
     - Please input your OpenAI API key in the environment variable above: `OPENAI_API_KEY`.
...
3. ***Retrieval Filtering (Agent):*** A 2nd layer "filter" of the chunks retrieved via vanilla RAG e.g., from 6 chunks reduced to 3 most relevant chunks, reducing noise.
4. ***Main Conversation (Agent):*** The main conversational agent to correspond with the user, citing the necessary sources accordingly.
...
Overall, for this case, as the dataset seems to be static, I did not code to scrape the documents and links on every code run (but it can be done so where needed in future).
...
UPDATE: As the dataset (see earlier section) does not include the infographics from https://www.mof.gov.sg/singaporebudget/resources/budget-infographics, the LLM was not able to share and cite the specific payout information in Dec 2024. Adding this infographic dataset is part of the future roadmap, as the data structure of the PDF files requires additional work to be parsed correctly for usage.
